"""Run random search on the GP samples."""
import os
import random
import numpy as np
import pandas as pd
...
def specify_experimental_configuration():
    ...
    exp_id = "random_threedimse_tmlr"
    params = parameters.Parameters(exp_id, kernelparams, domainparams, optimizerparams)
    return exp_id, params


def run_synthetic_experiment():
    """Run the experiments with random search"""
    random_id, params = specify_experimental_configuration()
    randomexp = RandomExperiment(params)
    randomexp.load_samples(
