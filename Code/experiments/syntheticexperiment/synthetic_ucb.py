"""Run GP-UCB on the GP samples for different choices of beta."""
import os
import random
import numpy as np
import pandas as pd
...
import parameters
from baselines.ucb_experiment import UcbExperiment
...
def specify_experimental_configuration(beta):
    ...
    exp_id = "ucb_threedimse" + "_discretization" + str(beta) + "_tmlr"
    params = parameters.Parameters(exp_id, kernelparams, domainparams, optimizerparams)
    return exp_id, params


def run_synthetic_experiment():
    """Run the experiments with UCB"""
    for discretization in [1, 10, 100, 1000]:
        ucb_id, params = specify_experimental_configuration(discretization)
        ucbexp = UcbExperiment(params)
        ucbexp.load_samples(
