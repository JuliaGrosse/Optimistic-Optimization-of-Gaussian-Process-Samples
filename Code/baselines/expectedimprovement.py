...
from emukit.core.interfaces import (
    IDifferentiable,
...
class ExpectedImprovement(Acquisition):
    def __init__(
        self, model: Union[IModel, IDifferentiable], jitter: float = 0.0
    ) -> None:
        """
...
class MultipointExpectedImprovement(ExpectedImprovement):
    ...
    def evaluate_with_gradients(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        qei_grad = self._get_acquisition_gradient(
            mean, variance, mean_dx, variance_dx, y_minimum, pk, symmetric_term
        )
        return -qei, -qei_grad

    def _get_acquisition(
        self, mu: np.ndarray, Sigma: np.ndarray, y_minimum: float
    ) -> Tuple:
        """
        ...
        for k in range(q):
            ...
            pk[k] = scipy.stats.multivariate_normal.cdf(
                b_k - mu_k, np.zeros((q,)), Sigma_k
            )
            ...
            ) = self._get_non_symmetric_and_symmetric_term_k(b_k, mu_k, Sigma_k, pk, k)
    ...
    def _get_acquisition_gradient(
        ...
        for k in range(q):
            ...
            (
                ...
                gradpk,
                hesspk,
            ) = self._gradient_of_the_acquisition_first_term(
        ...
        if self.fast_compute:
            non_symmetric_term[k] = (
                ...
                * (
                    scipy.stats.multivariate_normal.cdf(
                        b_k - mu_k + self.eps * Sigma_k[:, k].flatten(),
                        np.zeros((q,)),
                        Sigma_k,
                    )
                    - pk[k]
            ...
            for i in range(q):
                ...
                if i >= k:
                    ...
                    bik = b_k[i]
                    phi_ik = scipy.stats.norm.pdf(
                        bik, loc=mik, scale=np.sqrt(sigma_ii_k)
                    )
                    cik = get_correlations_given_value_of_i(
                        b_k, mu_k, Sigma_k, i
                    ).flatten()
                    sigmaik = get_covariance_given_value_of_i(Sigma_k, i)
                    Phi_ik = scipy.stats.multivariate_normal.cdf(
                        cik, np.zeros((q - 1,)), sigmaik
                    )
    ...
    def _gradient_of_the_acquisition_first_term(
        ...
        for l, j in itertools.product(range(q), range(d)):
            ...
            Dpk[l, j] = (
                0.5 * np.sum(hesspk * Sigk_dx[l, j, :, :])
                - gradpk[None, :] @ mk_dx[l, j, :, None]
            )
    ...
    def _gradient_of_the_acquisition_second_term(
        ...
        if self.fast_compute:
            ...
            gradpk1 = Phi_gradient(
                bk - mk + Sigk[:, k, None] * self.eps, np.zeros((q, 1)), Sigk
            )
            hesspk1 = Phi_hessian(
                bk + Sigk[:, k, None] * self.eps, mk, Sigk, gradient=gradpk1
            )
            for l, j in itertools.product(range(q), range(d)):
                f1 = (
                ...
                f = -(mk_dx[None, l, j, :] @ gradpk[:, None]) + 0.5 * np.sum(
                    Sigk_dx[l, j, :, :] * hesspk
                )
            ...
            for i in range(q):
                ...
                ck_pi = (
                    bk[ineq, 0]
                    - mk[ineq, 0]
                    - (bk[i, 0] - mk[i, 0]) / Sigk_ii * Sigk[ineq, :][:, i]
                )[:, None]
                Sigk_pi = 0.5 * (
                    Sigk[ineq, :][:, ineq]
                    - 1.0
                ...
                phi_ik = np.max(
                    [
                        scipy.stats.multivariate_normal.pdf(bk[i, 0], mk_i, Sigk_ii),
                        1e-11,
                    ]
                )
                dphi_ik_dSig = (
                    (bk_i - mk_i) ** 2 / (2.0 * Sigk_ii ** 2) - 0.5 / Sigk_ii
                ) * phi_ik
                ...
                for l, j in itertools.product(range(q), range(d)):
                    ...
                    B2[l, j] = (
                        B2[l, j]
                        + Sigk_ik
                        * (mk_dx_i[l, j] * dphi_ik_dm + dphi_ik_dSig * Sigk_dx_ii[l, j])
                        * Phi_ik
                    )
                    ...
                    B3[l, j] = B3[l, j] + Sigk_ik * phi_ik * (
                        (GPhi_ik.T @ dck_pi) + 0.5 * np.sum(HPhi_ik * dSigk_pi)
                    )
            B = B1 + B2 + B3
        return B
...
def get_correlations_given_value_of_i(
    b: np.ndarray, mu: np.ndarray, Sigma: np.ndarray, i: int
) -> np.ndarray:
    """
...
def decompose_mvn(
    x: np.ndarray, mu: np.ndarray, Sigma: np.ndarray, k: list
) -> np.ndarray:
    """
    ...
    if min(np.diag(varcov)) <= 0:
        varcov[range(len(neqk)), :][:, range(len(neqk))] = (
            np.diag(varcov) * (np.diag(varcov) >= 0) + 1e-9
        )
...
def Phi_hessian(
    x: np.ndarray, mu: np.ndarray, Sigma: np.ndarray, gradient: np.ndarray = None
) -> np.ndarray:
    """
    ...
    if gradient is None:
        res = res - np.diag(
            (
                (x - mu).flatten() * Phi_gradient(x, mu, Sigma)[:, None]
                + np.diag(Sigma @ res).flatten()
            )
            / np.diag(Sigma).flatten()
        )
    else:
        res = res - np.diag(
            ((x - mu).flatten() * gradient + np.diag(Sigma @ res).flatten())
            / np.diag(Sigma).flatten()
        )
    return res
